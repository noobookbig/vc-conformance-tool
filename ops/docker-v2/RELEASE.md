...
## v2.1.0 cut (mirror, source-only)

The v2.1.0 cut (see [MAS-304](/MAS/issues/MAS-304), 2026-06-11) mirrors
the v2.0.0 shape with the source-only distribution (the board's
`skip_ghcr` decision on [MAS-278](/MAS/issues/MAS-278) was inherited —
no GHCR image push). The tarball, the GitHub Release, the
`README.md` / `docker-compose.yml` quick-start, and the v2.1.0
CHANGELOG section follow the same recipe below with the version
bumps updated.

> **Packaging fix landed on v2.1.1** ([MAS-306](/MAS/issues/MAS-306),
> tag `v2.1.1` at commit `2f5d946` + version bumps). The v2.1.0
> tarball was missing `scripts/refresh-case-roles.sh` and
> `apps/conformance-v2/web/src/lib/data/case-roles.json` (both
> untracked in the working tree; the JSON was also under the
> blanket `data/` rule in `.gitignore`). v2.1.1 ships both files
> tracked, wires the script into the `ui-build` stage of the
> Dockerfile, and adds a `prebuild` npm script. Consumers who hit
> `TS2307: Cannot find module "./data/case-roles.json"` on the
> v2.1.0 tarball should re-pull from `v2.1.1` (the inline
> `node -e` workaround below is no longer needed).

**Workaround (v2.1.0 only, not needed on v2.1.1):**

```bash
tar -xzf vc-conformance-v2-2.1.0.tar.gz
cd vc-conformance-v2-2.1.0
# The script is not in the tarball — generate the JSON inline with
# the same shape the script produces. (The script lives in the
# working repo at scripts/refresh-case-roles.sh but is untracked.)
mkdir -p apps/conformance-v2/web/src/lib/data
node -e '
  const fs = require("fs"), path = require("path");
  const dir = "references/testcases";
  const map = {};
  for (const f of fs.readdirSync(dir).filter(f => f.endsWith(".yaml"))) {
    const txt = fs.readFileSync(path.join(dir, f), "utf8");
    const id = (txt.match(/^id:\s*(.+)$/m) || [])[1];
    const eut = (txt.match(/^eut:\s*(.+)$/m) || [])[1];
    if (!id) continue;
    let role = "multi";
    if (eut === "issuer") role = "issuer";
    else if (eut === "verifier") role = "verifier";
    else if (eut === "holder" || eut === "wallet") role = "wallet";
    else if (eut === "resolver") role = "resolver";
    map[id] = role;
  }
  fs.writeFileSync("apps/conformance-v2/web/src/lib/data/case-roles.json",
    JSON.stringify({ _meta: { source: "references/testcases/ (v2.0 catalog)",
      regenerate: "Run scripts/refresh-case-roles.sh in repo root." },
      ...map }, null, 2) + "\n");
  console.log("Wrote", Object.keys(map).length, "case roles.");
'
npm --prefix apps/conformance-v2/web run build
docker build -f ops/docker-v2/Dockerfile -t vc-conformance-v2:2.1.0 .
```
...
- A patch release (v2.0.1, v2.1.1) increments `version`, re-tags,
  and ships. The `CHANGELOG.md` gets a new section above the
  previous patch (e.g. `## v2.1.1` above `## v2.1.0`).
