...
**Packaging gap to be aware of (see [MAS-306](/MAS/issues/MAS-306)):**

[MAS-296](/MAS/issues/MAS-296) (3fcf390, v2 web UI — visual role
separation) introduced `apps/conformance-v2/web/src/lib/roles.ts`
which imports a build-time JSON file
(`apps/conformance-v2/web/src/lib/data/case-roles.json`). The JSON
is **gitignored** (the `data/` pattern in `.gitignore` covers it) and
**generated by a script that is not in git**
(`scripts/refresh-case-roles.sh`, also untracked). The v2.1.0 tag
(`4c28c0d` → commit `1653b5e`) ships the import without the
imported file, so a clean tarball extraction fails the `ui-build`
stage of the Dockerfile (`TS2307: Cannot find module
"./data/case-roles.json"`).

The v2.0.0 tarball does **not** hit this — MAS-296 landed after the
v2.0.0 cut, so v2.0.0 never imported the JSON. The v2.1.0 cut
inherits the gap.

**Workaround (consumer side, until MAS-306 lands):**
...
**The v2.1.1 cut (recommended):** fix the packaging properly —
commit the script + the JSON, add a `RUN bash scripts/refresh-case-roles.sh`
pre-step in the Dockerfile's `ui-build` stage, drop the `data/`
pattern from `.gitignore`, and ship a v2.1.1 patch. Tracked in
[MAS-306](/MAS/issues/MAS-306).
